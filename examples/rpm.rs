//! Test bundler for RPM platform
//!
//! This example tests the bundler binary by invoking it as a subprocess
//! and building a package for Fedora/RHEL/CentOS.
//!
//! Output written to ./tmp/test_rpm.log
//! Bundle artifact: ./tmp/bundle/{package_id}.rpm
//!
//! Run with: cargo run --example rpm -- --source owner/repo
...
fn main() {
    ...
    let source = if let Some(pos) = args.iter().position(|arg| arg == "--source") {
        ...
        eprintln!("Usage: cargo run --example rpm -- --source owner/repo");
        std::process::exit(1);
    };
