//! Test bundler for DEB platform
//!
//! This example tests the bundler binary by invoking it as a subprocess
//! and building a package for Debian/Ubuntu.
//!
//! Output written to ./tmp/test_deb.log
//! Bundle artifact: ./tmp/bundle/{package_id}.deb
//!
//! Run with: cargo run --example deb -- --source owner/repo
...
fn main() {
    ...
    let source = if let Some(pos) = args.iter().position(|arg| arg == "--source") {
        ...
        eprintln!("Usage: cargo run --example deb -- --source owner/repo");
        std::process::exit(1);
    };
