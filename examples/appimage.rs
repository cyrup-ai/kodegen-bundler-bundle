//! Test bundler for AppImage platform
//!
//! This example tests the bundler binary by invoking it as a subprocess
//! and building a portable Linux AppImage.
//!
//! Output written to ./tmp/test_appimage.log
//! Bundle artifact: ./tmp/bundle/{package_id}.AppImage
//!
//! Run with: cargo run --example appimage -- --source owner/repo

mod common;

fn main() {
    let platform = "appimage";
    ...
    let source = if let Some(pos) = args.iter().position(|arg| arg == "--source") {
        ...
        eprintln!("Usage: cargo run --example appimage -- --source owner/repo");
        std::process::exit(1);
    };
