...
pub async fn bundle_project(settings: &Settings) -> Result<Vec<PathBuf>> {
    ...
    copy_custom_files(&settings.bundle_settings().deb.files, &data_dir)
        .await
...
async fn generate_desktop_file(settings: &Settings, data_dir: &Path) -> Result<()> {
    ...
    tokio::task::spawn_blocking(move || -> Result<()> {
        if let Some(parent) = desktop_path.parent() {
            std_fs::create_dir_all(parent).fs_context("creating desktop file directory", parent)?;
        }
        let mut file =
            File::create(&desktop_path).fs_context("creating desktop file", &desktop_path)?;
...
async fn generate_changelog(settings: &Settings, data_dir: &Path) -> Result<()> {
    if let Some(changelog_path) = &settings.bundle_settings().deb.changelog {
        ...
        tokio::task::spawn_blocking(move || -> Result<()> {
            let mut src = File::open(&src_path).fs_context("opening changelog file", &src_path)?;
...
async fn generate_control_file(
    ...
    tokio::task::spawn_blocking(move || -> Result<()> {
        if let Some(parent) = control_path.parent() {
            std_fs::create_dir_all(parent).fs_context("creating control directory", parent)?;
        }
        let mut file =
            File::create(&control_path).fs_context("creating control file", &control_path)?;
        ...
        let short = short_description.as_deref().unwrap_or("(no description)");
...
async fn generate_md5sums(control_dir: &Path, data_dir: &Path) -> Result<()> {
    ...
    tokio::task::spawn_blocking(move || -> Result<()> {
        if let Some(parent) = md5sums_path.parent() {
            std_fs::create_dir_all(parent).fs_context("creating md5sums directory", parent)?;
        }
        let mut file =
            File::create(&md5sums_path).fs_context("creating md5sums file", &md5sums_path)?;
        ...
        for entry in WalkDir::new(&data_dir) {
            ...
            // Calculate MD5 hash
            let mut src =
                File::open(entry.path()).fs_context("opening file for MD5", entry.path())?;
...
async fn generate_scripts(settings: &Settings, control_dir: &Path) -> Result<()> {
    ...
    for (script_opt, name) in scripts {
        if let Some(script_path) = script_opt {
            ...
            let mut src = tokio::fs::File::open(script_path)
                .await
...
async fn tar_and_gzip_dir(src_dir: PathBuf) -> Result<PathBuf> {
    ...
    let tar_gz = tokio::fs::File::create(&dest_path)
        .await
