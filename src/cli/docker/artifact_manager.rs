...
impl ArtifactManager {
    ...
    pub fn discover_artifacts(
        ...
        for entry in entries {
            ...
            if self.is_valid_artifact(&path, platform) {
        ...
        if artifacts.is_empty() {
            return Err(self.format_no_artifacts_error(&bundle_dir, platform)?);
        }

        Ok(artifacts)
    }

    /// Checks if a file is a valid artifact for the given platform.
    fn is_valid_artifact(&self, path: &PathBuf, platform: PackageType) -> bool {
        let extension = path
            .extension()
            .and_then(|e| e.to_str())
    ...
    pub fn move_artifacts_to_final(
        &self,
